...
/**
 * The API controller for Declined Transction related operations performed by Acountant.
...
if(http_response_code() === 200){
    ...
    if($action == "view"){
        ...
        elseif(isset($key) && isset($value)){
            ...
            $search = array($key => $value, "status" => "declined");
            ...
            $login = new database\AccountTransactionAccess(new database\SQLHandler($db->conn));
            $filters = null;
            $search = array("status" => "declined", "office" => $GLOBALS['office']);
            $result = $login->selectWithClause($filters,$search, 'AND', $cmd);
            $login->close();
            echo utilities\JSONHandler::arrayToJSON($result);
        }
