...
define('MODULE', 'Accountant'); //Module name

//include the general required file
require_once '../required.php';

define('APP_PATH', ROOT_DIR.'/modules/accountant'); //Application path
...
$klein->with('/api', function () use ($klein) {
    ...
    $klein->respond(array('POST','GET'),'/[a:ctrl]/[i:id]', function ($request, $response) {
        ...
        elseif(strcasecmp($request->ctrl, 'customer') === 0 ){
    ...
    $klein->respond(array('POST','GET'),'/[a:ctrl]/[delete|edit:action]/[i:id]', function ($request, $response) {
        //perform action on a single  with id
        $id = $request->id;
        $action = $request->action;
        $data = json_decode(json_decode($request->body()));
        if(strcasecmp($request->ctrl, 'pending') === 0 ){
