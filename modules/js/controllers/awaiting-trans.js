...
user.controller("awaitingCtrl", function($scope, $location, $route, httpReq, showAlert){
    ...
    $scope.searchBtn = function(){
        if($scope.key != null && $scope.value != null){
            $location.url('/awaiting-trans?key=' + $scope.key + '&value=' + $scope.value);
        }
        else{
    ...
    $scope.confirmBtn = function(){
        if(confirm('Change of status confirmation for this transaction. Note: This action can not be reversed when done')){
            httpReq.send('/awaiting/edit/' + $scope.selected_trans.id, JSON.stringify($scope.selected_trans), 'POST',
            {
                success: function(response){
                    if(response.status === 200){
    ...
    $scope.next = function(){
        ...
        if($location.search().value && $location.search().key){
            $location.url('/awaiting-trans?key=' + key + '&value=' + value + "&page=" + new_page);
        }
        else{
            $location.url('/awaiting-trans?page=' + new_page);
        }
    }
    $scope.prev = function(){
        var new_page = parseInt(page) - 1;
