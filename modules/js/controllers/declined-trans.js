...
user.controller("declinedCtrl", function($scope, $location, $route, httpReq, showAlert){
    ...
    $scope.searchBtn = function(){
        if($scope.key != null && $scope.value != null){
            $location.url('/declined-trans?key=' + $scope.key + '&value=' + $scope.value);
        }
        else{
    ...
    $scope.next = function(){
        ...
        if($location.search().value && $location.search().key){
            $location.url('/declined-trans?key=' + key + '&value=' + value + "&page=" + new_page);
        }
        else{
            $location.url('/declined-trans?page=' + new_page);
        }
    }
    $scope.prev = function(){
        var new_page = parseInt(page) - 1;
