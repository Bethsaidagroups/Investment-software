...
user.controller("creditCtrl", function($scope, $location, $route, httpReq, showAlert){
    ...
    $scope.searchBtn = function(){
        if($scope.key != null && $scope.value != null){
            $location.url('/credit-trans?key=' + $scope.key + '&value=' + $scope.value);
        }
        else{
    ...
    $scope.next = function(){
        ...
        if($location.search().value && $location.search().key){
            $location.url('/credit-trans?key=' + key + '&value=' + value + "&page=" + new_page);
        }
        else{
            $location.url('/credit-trans?page=' + new_page);
        }
    }
    $scope.prev = function(){
        var new_page = parseInt(page) - 1;
